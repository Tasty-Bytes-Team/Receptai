...
async function fetchRecipes() {
  ...
  try {
    ...
    recipeList.value = response.data.elements.filter(
...
<template>
  <div class="container mx-auto py-8">
    ...
    <h1 class="text-sm lg:text-2xl md:text-lg sm:text-sm">
      Welcome back, {{ TastyBytes_user.user.name }}!
    </h1>
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <!-- Placeholder div to push content to the right on all sizes -->
      <div class="py-8 col-span-1 lg:col-span-2">All needed info here</div>
      <div class="col-span-1">
        ...
        >
          <div
            class="font-roboto-mono font-bold flex justify-center items-center"
          >
            {{ currentTime }}
          </div>
        </Widget>
          ...
          <div v-if="recipeList.length > 0">
            <div class="flex-shrink-0 mt-2">
              <RecipeBannerComponent
                :imageLink="recipeList[0].previewImage"
                :name="recipeList[0].name"
                :about="recipeList[0].shortDescription"
                :link="`/recipes/${recipeList[0].id}`"
              />
            </div>
          </div>
