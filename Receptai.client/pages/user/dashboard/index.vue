...
function startStopwatch() {
  if (!stopwatch.value.isRunning) {
    stopwatch.value.startTime = new Date().getTime() - stopwatch.value.elapsedTime;
    stopwatch.value.isRunning = true;
    stopwatch.value.intervalId = setInterval(updateStopwatch, 10);
  }
}
function stopStopwatch() {
  stopwatch.value.elapsedTime = new Date().getTime() - stopwatch.value.startTime;
...
async function fetchRecipes() {
  ...
  try {
    ...
    const userRecipes = response.data.elements.filter((recipe: Recipe) => recipe.author.id === TastyBytes_user.value.user.id);
    
    userRecipes.sort((a: Recipe, b: Recipe) => new Date(b.dateCreated).getTime() - new Date(a.dateCreated).getTime());
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div class="col-span-1 sm:col-span-2 lg:col-span-2">
          <div class="bg-white rounded-lg shadow-md p-6 flex items-center justify-center" >
            <Widget class="border" orientation="horizontal" title="Your recipe count is">
